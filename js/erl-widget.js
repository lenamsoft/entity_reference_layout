(function($, Drupal, drupalSettings, debounce, displace) {
    ...
    function resetSize(event) {
        if ($('.ui-dialog').length > 0) {
            ...
            //event.data.$element.dialog('close');
        }
    ...
    $(window).on({
        'dialog:aftercreate': function dialogAftercreate(event, dialog, $element, settings) {
            ...
            if (settings.autoResize === 'yes') {
                $element.dialog('option', { resizable: false, draggable: false }).dialog('widget').css('position', 'fixed');
                $(window).on('resize.dialogResize scroll.dialogResize', eventData, autoResize).trigger('resize.dialogResize');
                // $(document).on('drupalViewportOffsetChange.dialogResize', eventData, autoResize);
            }
        },
        'dialog:beforeclose': function dialogBeforeclose(event, ui) {
    ...
    Drupal.behaviors.erlWidget = {
        attach: function attach(context, settings) {
            ...
            $('.erl-field .erl-form', context).once('erl-dialog').each(function(index, item) {
                var dl_title = '';
                if ($(item).find('[data-dialog-title]').length > 0) {
                    dl_title = $(item).find('[data-dialog-title]').attr('data-dialog-title');
                }
                var dialogConfig = {
                    width: '90%',
                    title: dl_title,
                    ...
                    beforeClose: function(event, ui) {
                        $(event.target).find('.erl-cancel')
                            .trigger('mousedown', [{ fromClose: true }])
                            .trigger('click', [{ fromClose: true }]);
                    }


                };

                var dialog = Drupal.dialog('.erl-form', dialogConfig);
                dialog.showModal();
            });
